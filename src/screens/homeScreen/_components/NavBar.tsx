    ...
    <div className="w-[100%] h-auto flex">
          ...
          {navbarData.map((data) => (
            <div key={data} className="z-1">
              <Typography
                ...
                text={data}
                classname={
                  "flex-initial hover:cursor-pointer hover:rounded-md z-1 hover:z-50 hover:visible hover:scale-125 pointer-events-auto transition ease-in-out duration-300  hover:font-semibold hover:underline hover:px-2 hover:py-1 hover:underline-offset-8"
                }
              />
            </div>
          ))}
      ...
      <AnimatePresence mode="wait">
        {isMenuClose ? (
          ...
          <motion.div
            key="full-menu"
            className="flex flex-col w-full relative md:hidden"
